...
def find_chrome():
    # Playwright's bundled Chromium (present in the remote build environment)
    import glob
    for candidate in sorted(glob.glob("/opt/pw-browsers/chromium-*/chrome-linux/chrome"), reverse=True):
    ...
    try:
        for page in pages:
            for label, width, height in VIEWPORTS:
                target = out / f"{page.replace('/', '-').rsplit('.', 1)[0]}-{label}.png"
                subprocess.run([
                    chrome,
                    "--headless=new",
                    "--disable-gpu",
